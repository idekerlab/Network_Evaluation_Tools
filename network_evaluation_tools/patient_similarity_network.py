...
# Pairwise cosine correlation of rows in given data
def pairwise_cosine(propagated_sm_matrix):
    tmp = propagated_sm_matrix.as_matrix()
    cos = np.dot(tmp, tmp.T)
    
    # Cosine
    marg_norms = np.sqrt(np.array((tmp * tmp).sum(1)).flatten())
    cos = cos / marg_norms.reshape(1, -1)
    cos = cos / marg_norms.reshape(-1, 1)    
    cos_df = pd.DataFrame(cos, index=propagated_sm_matrix.index, columns=propagated_sm_matrix.index)
    return cos_df
